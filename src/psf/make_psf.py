...
#Had to update from from photutils import EPSFBuilder because deprication was causing an issue with of the functions.
...
for field in fields:
    ...
    for fi in filters:
        ...
        stars = extract_stars(nddata, atstar, size=69)

        # Correct: Extracting the data and flux from each EPSFStar object
        star_data = [star.data for star in stars]  # Extract the data from each star
        fluxes = [np.sum(star.data) for star in stars]  # Calculate the flux for each star

        pstarobj = pd.DataFrame(data={'star_obj': star_data, 'flux': fluxes}, dtype='object')
        ...
        stars = extract_stars(nddata,atstar[bright_idx], size=(69,69))
