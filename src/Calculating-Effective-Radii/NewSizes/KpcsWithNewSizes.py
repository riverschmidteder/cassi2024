...
# Provided arcsecond values and their corresponding redshifts
arcsecs = [
...
# Loop through each galaxy's arcsecond and redshift value
for arcsec, z in zip(arcsecs, redshifts):
    if not np.isnan(arcsec) and not np.isnan(z):
        Re_kpc = cosmo.kpc_proper_per_arcmin(z).value * arcsec * u.arcsec.to(u.arcmin)
        Re_kpc_values.append(Re_kpc)
    else:
        Re_kpc_values.append(np.nan)

# Assuming you have already run the loop that calculates Re_kpc_values
Re_kpc_values = []
...
# Display the results
for i, (arcsec, z, Re_kpc) in enumerate(zip(arcsecs, redshifts, Re_kpc_values)):
    print(f"Galaxy {i+1}: Arcsec = {arcsec:.6f}, Redshift = {z:.6f}, Re_kpc = {Re_kpc:.6f} kpc")
