...
# If no VDW match, use SEx catalogs for magnitude and re
...
for row in pcrs.itertuples():
    for run in run_strs:
        ...
        elif not pd.isnull(getattr(row, '{}_SEx_ID'.format(fi))): # Otherwise, use SEx catalog, if available
